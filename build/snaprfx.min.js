...
c.canvas.context.drawImage(c.original.canvas,0,0);c.update_element();setTimeout(function(){c.create_overlay_elements();c.deferred.resolve()},1)})});c.load_filter_pack=new Deferred;var a=new XMLHttpRequest;a.onload=function(){c.filter_pack=JSON.parse(a.response).filter_pack;c.filter_pack.base_path=c.options.filter_pack;c.load_filter_pack.resolve(c.filter_pack);c.load_fonts()};a.open("get",c.options.filter_pack+"filter-pack.json",!0);a.send();if(c.options.sticker_pack){c.load_sticker_pack=new Deferred;
var d=new XMLHttpRequest;d.onload=function(){c.sticker_pack=JSON.parse(d.response).sticker_pack;c.sticker_pack.target_canvas||(c.sticker_pack.target_canvas={width:800,height:800});c.sticker_pack.by_slug={};c.sticker_pack.base_path=c.options.sticker_pack;c.sticker_pack.sections.forEach(function(a){a.stickers.forEach(function(a){c.sticker_pack.by_slug[a.slug]=a})});c.load_sticker_pack.resolve(c.sticker_pack)};d.open("get",c.options.sticker_pack+"sticker-pack.json",!0);d.send()}c.current_filter="_original";
c.filter_specs={_original:{name:"*Original*",slug:"_original",layers:[]}};c.stickers=[];c.text=[]};SnaprFX.prototype.set_url=function(b,c){var a=this;this.options.url=b;var d=new Deferred;SnaprFX.utils.read_exif(a.options.url).done(function(c){a.exif=c;a.load_original(!0).done(function(){a.apply_filter({editable:a.render_options.editable});d.resolve()})});a.without_extras=null;return d};
