...
a.send();return c},rgbToHsl:function(b,c,a){b/=255;c/=255;a/=255;var d=Math.max(b,c,a),f=Math.min(b,c,a),h,g=(d+f)/2;if(d==f)h=f=0;else{var e=d-f,f=0.5<g?e/(2-d-f):e/(d+f);switch(d){case b:h=(c-a)/e+(c<a?6:0);break;case c:h=(a-b)/e+2;break;case a:h=(b-c)/e+4}h/=6}return[h,f,g]},hslToRgb:function(b,c,a){if(0===c)a=c=b=a;else{var d=0.5>a?a*(1+c):a+c-a*c,f=2*a-d;a=SnaprFX.utils.hueToRgb(f,d,b+1/3);c=SnaprFX.utils.hueToRgb(f,d,b);b=SnaprFX.utils.hueToRgb(f,d,b-1/3)}return[255*a,255*c,255*b]},hueToRgb:function(b,
