...
c.canvas.context.drawImage(c.original.canvas,0,0);c.update_element();setTimeout(function(){c.create_overlay_elements();c.deferred.resolve()},4)})});c.load_filter_pack=new Deferred;var a=new XMLHttpRequest;a.onload=function(){c.filter_pack=JSON.parse(a.response).filter_pack;c.filter_pack.base_path=c.options.filter_pack;c.filter_pack.by_slug={};c.filter_pack.sections.forEach(function(a){a.filters.forEach(function(b){b.section=a;c.filter_pack.by_slug[b.slug]=b})});c.load_filter_pack.resolve(c.filter_pack);
c.load_fonts()};a.open("get",c.options.filter_pack+"filter-pack.json",!0);a.send();if(c.options.sticker_pack){c.load_sticker_pack=new Deferred;var d=new XMLHttpRequest;d.onload=function(){c.sticker_pack=JSON.parse(d.response).sticker_pack;c.sticker_pack.target_canvas||(c.sticker_pack.target_canvas={width:800,height:800});c.sticker_pack.by_slug={};c.sticker_pack.base_path=c.options.sticker_pack;c.sticker_pack.sections.forEach(function(a){a.stickers.forEach(function(a){c.sticker_pack.by_slug[a.slug]=
a})});c.load_sticker_pack.resolve(c.sticker_pack)};d.open("get",c.options.sticker_pack+"sticker-pack.json",!0);d.send()}c.current_filter="_original";c.filter_specs={_original:{name:"*Original*",slug:"_original",layers:[]}};c.stickers=[];c.text=[]};
SnaprFX.prototype.set_url=function(b,c){var a=this;this.options.url=b;var d=new Deferred;SnaprFX.utils.read_exif(a.options.url).done(function(c){a.exif=c;a.load_original(!0).done(function(){a.apply_filter({editable:a.render_options.editable});d.resolve()})});a.without_extras=null;return d};
...
SnaprFX.prototype.apply_filter=function(b){var c=this;!c.deferred||!c.deferred.resolved?console.warn("Not ready yet"):(c.deferred=new Deferred,dom.addClass(document.body,"fx-processing"),setTimeout(function(){function a(){var a=c.filter_specs[c.current_filter];a&&(a.layer_index=-1);if(c.render_options.region)debug_logging&&console.log("region",c.render_options.region),a=c.render_options.region,c.canvas.context.drawImage(c.original.canvas,a.left,a.top,a.width,a.height,a.left,a.top,a.width,a.height);
else{var d=function(){c.canvas.context.drawImage(c.original.canvas,0,0);var a=function(){c.pixels=c.canvas.get_data();c.apply_next_layer()};!b.editable||c.options.disable_sticker_edit?c.render_stickers().done(a):a()};c.original.set_size(c.render_options).done(function(){c.canvas.set_size({width:c.original.width,height:c.original.height}).done(d)})}}b=b||{};var d={filter:c.current_filter,editable:!1,width:c.options.width,height:c.options.height,size:c.options.size},f;for(f in d)f in b||(b[f]=d[f]);
c.render_options=b;!b.editable&&!b.output&&c.deferred.done(function(){(!1!==c.options.render_text||c.render_options.render_text)&&c.text.forEach(function(a){a.rerender()});c.stickers.forEach(function(a){a.rerender()})});if(b.filter!=c.current_filter){c.elements.old_text=[];d=c.elements.overlay.getElementsByClassName("fx-text");for(f=d.length-1;0<=f;f--)c.elements.old_text.push(d[f]);c.without_extras=null}debug_logging&&console.group(b.filter);"_original"!==b.filter&&(c.filter_specs[b.filter].details.settings&&
c.filter_specs[b.filter].details.settings.locked||c.filter_specs[b.filter].details.section.settings&&c.filter_specs[b.filter].details.section.settings.locked)?(c.elements.locked.style.display="block",c.elements.locked_text.innerText="Bonus items are only available in the app!"):c.elements.locked.style.display="none  ";(c.current_filter=b.filter)?b.filter in c.filter_specs?setTimeout(a,4):c.load_filter(b.filter).done(function(){setTimeout(a,4)}):setTimeout(a,4)},4))};
SnaprFX.prototype.load_filter=function(b){var c=this,a=c.filter_pack.by_slug[b],d=new Deferred,f=new XMLHttpRequest;f.onload=function(){debug_logging&&console.log("loaded",b);c.filter_specs[b]=JSON.parse(f.response).filter;c.filter_specs[b].target_canvas||(c.filter_specs[b].target_canvas={width:800,height:800});c.filter_specs[b].details=a;d.resolve()};f.open("get",c.filter_pack.base_path+"filters/"+b+"/filter.json",!0);f.send();return d};
SnaprFX.prototype.output=function(b){var c=this,a=new Deferred;c.apply_filter({output:!0,width:b.width,height:b.height,size:b.size});c.deferred.done(function(){a.resolve(c.canvas.get_data_url())});a.done(function(){c.original.set_size(c.options).done(function(){c.canvas.set_size({width:c.original.width,height:c.original.height})})});return a};
...
b.render_options.region||{left:0,top:0,width:b.canvas.width,height:b.canvas.height},g,k=d.top;k<d.top+d.height;k+=1)for(var m=d.left;m<d.left+d.width;m+=1){g=4*(k*b.canvas.width+m);var n;n=a.filter.whole_canvas?[f.data[g],f.data[g+1],f.data[g+2],f.data[g+3]]:a.filter.process(g,[b.pixels.data[g],b.pixels.data[g+1],b.pixels.data[g+2]]);var p=n[O],p=0<=p?p/255:1;a.mask_image&&(p*=h.data[g]/255);p*=(a.opacity||100)/100;n=a.blender.process([b.pixels.data[g],b.pixels.data[g+1],b.pixels.data[g+2]],[n[R],
n[G],n[B]],p);b.pixels.data[g]=n[R];b.pixels.data[g+1]=n[G];b.pixels.data[g+2]=n[B]}debug_logging&&console.timeEnd("applying "+c.layers[c.layer_index].type+" layer");b.apply_next_layer()}var f;a.filter.whole_canvas&&(0!==c.layer_index&&b.canvas.put_data(b.pixels),a.filter.process(b.canvas),f=b.canvas.get_data());var h;if(a.mask_image){var g=new SnaprFX.Canvas({url:b.filter_pack.base_path+"filters/"+c.slug+"/"+a.mask_image,width:b.canvas.width,height:b.canvas.height});g.deferred.done(function(){h=
g.get_data();d()})}else d()})}};
...
b.elements.locked=dom.div("fx-locked-layer");dom.setStyle(d,{display:"none",position:"absolute",top:0,left:0,height:"100%",width:"100%"});var f=b.elements.locked_text=dom.div("fx-locked-text");d.appendChild(f);b.elements.image.parentNode.insertBefore(a,b.elements.image.nextSibling);a.appendChild(b.elements.image);a.appendChild(c);a.appendChild(d);debug_canvas&&dom.addClass(c,"fx-debug")};SnaprFX.prototype.on=function(b,c){this.handlers[b]||(this.handlers[b]=[]);this.handlers[b].push(c)};
SnaprFX.prototype.trigger=function(b,c,a){this.handlers[b]&&this.handlers[b].forEach(function(b){b.call(c,a)})};"function"===typeof define&&define.amd&&define(function(){return SnaprFX});var dom={div:function(b){var c=document.createElement("div");c.className=b;return c},hasClass:function(b,c){return b.className.match(RegExp("(^|\\s)"+c+"(\\s|$)"))},addClass:function(b,c){dom.hasClass(b,c)||(b.className+=" "+c)},removeClass:function(b,c){b.className=b.className.replace(RegExp("(^|\\s)"+c+"(\\s|$)")," ")},setStyle:function(b,c){for(var a in c)b.style[a]=c[a]}};function Deferred(){this.callbacks=[]}
...
a.type){var b=new Image;b.src=c+a.image.image}a.mask_image&&(b=new Image,b.src=c+a.mask_image)})};d.open("get",c+"filter.json",!0);d.send()})})};c.open("get",b.filter_pack+"filter-pack.json",!0);c.send();var a=new XMLHttpRequest;a.onload=function(){JSON.parse(a.response).sticker_pack.sections.forEach(function(a){a.stickers.forEach(function(a){(new Image).src=b.sticker_pack+"assets/"+a.slug+".png"})})};a.open("get",b.sticker_pack+"sticker-pack.json",!0);a.send()},read_exif:function(b){var c=new Deferred,
a=new XMLHttpRequest;a.open("GET",b,!0);a.responseType="arraybuffer";a.onload=function(b){var f=new FileReader;f.onloadend=function(){var b,d;try{b=new JpegMeta.JpegFile(this.result,"photo.jpg"),d={latitude:b.gps&&b.gps.latitude&&b.gps.latitude.value,longitude:b.gps&&b.gps.longitude&&b.gps.longitude.value,date:b.exif&&b.exif.DateTimeOriginal&&b.exif.DateTimeOriginal.value,orientation:b.tiff&&b.tiff.Orientation&&b.tiff.Orientation.value}}catch(e){d={}}a=f=null;c.resolve(d)};b=new Blob([this.response]);
f.readAsBinaryString(b)};a.send();return c},rgbToHsl:function(b,c,a){b/=255;c/=255;a/=255;var d=Math.max(b,c,a),f=Math.min(b,c,a),h,g=(d+f)/2;if(d==f)h=f=0;else{var e=d-f,f=0.5<g?e/(2-d-f):e/(d+f);switch(d){case b:h=(c-a)/e+(c<a?6:0);break;case c:h=(a-b)/e+2;break;case a:h=(b-c)/e+4}h/=6}return[h,f,g]},hslToRgb:function(b,c,a){if(0===c)a=c=b=a;else{var d=0.5>a?a*(1+c):a+c-a*c,f=2*a-d;a=SnaprFX.utils.hueToRgb(f,d,b+1/3);c=SnaprFX.utils.hueToRgb(f,d,b);b=SnaprFX.utils.hueToRgb(f,d,b-1/3)}return[255*
a,255*c,255*b]},hueToRgb:function(b,c,a){0>a&&(a+=1);1<a&&(a-=1);return a<1/6?b+6*(c-b)*a:0.5>a?c:a<2/3?b+6*(c-b)*(2/3-a):b}};SnaprFX.prototype.add_sticker=function(b){var c=this;if(!c.deferred||!c.deferred.resolved)console.warn("Not ready yet");else{var a=new SnaprFX.sticker(b,c);c.stickers.push(a);a.deferred.done(function(){c.elements.overlay.appendChild(a.element);for(var b=c.elements.overlay.getElementsByClassName("fx-active"),f=0;f<b.length;f++)dom.removeClass(b[f],"fx-active");dom.addClass(a.element,"fx-active")});c.render_options.editable=!0}};
...
SnaprFX.sticker=function(b,c){var a=this;a.slug=b;a.parent=c;a.scale=1;a.rotation=0;a.spec=c.sticker_pack.by_slug[b];a.deferred=new Deferred;a.load().done(function(){a.scale_factor=Math.min(c.canvas.width/c.sticker_pack.target_canvas.width,c.canvas.height/c.sticker_pack.target_canvas.height);a.element=dom.div("fx-sticker fx-sticker-rendered");var d=function(a){a.stopPropagation();c.active_text&&c.active_text.deactivate()};a.element.addEventListener("click",d);a.element.addEventListener("touch",d);
a.element.innerHTML='<img class="fx-sticker-image" src="'+a.parent.sticker_pack.base_path+"assets/"+b+'.png">';d={position:"absolute",width:a.image.width*a.scale_factor,height:a.image.height*a.scale_factor};a.spec.position&&a.spec.position.x?(d.left=a.spec.position.x*a.scale_factor-d.width/2,d.top=a.spec.position.y*a.scale_factor-d.height/2):(d.left=c.canvas.width/2-d.width/2,d.top=c.canvas.height/2-d.height/2);d.width+="px";d.height+="px";d.left+="px";d.top+="px";dom.setStyle(a.element,d);a.initial=
d;a.rendered||dom.removeClass(a.element,"fx-sticker-rendered");a.element.innerHTML+='<a class="fx-render-sticker fx-sticker-handle" data-role="button" data-icon="tick" data-iconpos="notext" data-theme="e">\u2714</a>';setTimeout(function(){var b=function(a){c.rerender_editables();a.stopPropagation()},d=a.element.getElementsByClassName("fx-render-sticker")[0];d.addEventListener("click",b);d.addEventListener("touch",b)},4);a.element.innerHTML+='<a class="dd fx-remove-sticker fx-sticker-handle" data-role="button" data-icon="delete" data-iconpos="notext" data-theme="e">\u2717</a>';
setTimeout(function(){var c=a.element.getElementsByClassName("fx-remove-sticker")[0];c.addEventListener("click",function(){a.remove()});c.addEventListener("touch",function(){a.remove()})},4);d=function(){if(!a.parent.options.disable_sticker_edit){a.rendered&&c.unrender_editables();for(var b=c.elements.overlay.getElementsByClassName("fx-active"),d=0;d<b.length;++d)dom.removeClass(b[d],"fx-active");dom.addClass(a.element,"fx-active");dom.removeClass(a.element,"fx-sticker-rendered")}};a.element.addEventListener("mousedown",
d);a.element.addEventListener("touchstart",d);a.mousemove_drag=function(c){a.rendered||dom.setStyle(a.element,{left:c.pageX-a.drag_from.left+"px",top:c.pageY-a.drag_from.top+"px"})};a.mouseup=function(){c.elements.wrapper.removeEventListener("mousemove",a.mousemove_drag);c.elements.wrapper.removeEventListener("mousemove",a.mousemove_scale);c.elements.wrapper.removeEventListener("touchmove",a.mousemove_drag);c.elements.wrapper.removeEventListener("touchmove",a.mousemove_scale)};c.elements.wrapper.addEventListener("mouseup",
a.mouseup);c.elements.wrapper.addEventListener("touchend",a.mouseup);d=function(b){if(!a.rendered){var d=parseInt(a.element.style.left,10),g=parseInt(a.element.style.top,10);a.drag_from={left:b.pageX-d,top:b.pageY-g,event:b};c.elements.wrapper.addEventListener("mousemove",a.mousemove_drag);c.elements.wrapper.addEventListener("touchmove",a.mousemove_drag)}dom.addClass(c.elements.overlay,"fx-editing-sticker");dom.removeClass(c.elements.overlay,"fx-editing-text")};a.element.addEventListener("mousedown",
d);a.element.addEventListener("touchstart",d);a.mousemove_scale=function(c){c.stopPropagation();if(!a.rendered){var b=SnaprFX.utils.pythag(c.pageX-a.scale_from.x,c.pageY-a.scale_from.y);a.scale=b/a.scale_from.size;a.scale=Math.max(a.scale,0.15);c=SnaprFX.utils.cart2polar(c.pageX-a.scale_from.x,c.pageY-a.scale_from.y);a.rotation=-a.scale_from.rotation+c;c=parseInt(a.initial.width,10)*a.scale;var b=parseInt(a.initial.height,10)*a.scale,d=a.parent.elements.overlay.getBoundingClientRect();dom.setStyle(a.element,
{webkitTransform:"rotate("+Math.round(360*(a.rotation/(2*Math.PI)))+"deg)",transform:"rotate("+Math.round(360*(a.rotation/(2*Math.PI)))+"deg)",width:c+"px",height:b+"px",top:a.scale_from.y-b/2-d.top-window.pageYOffset+"px",left:a.scale_from.x-c/2-d.left-window.pageXOffset+"px"})}};a.element.innerHTML+='<a class="fx-scale-sticker fx-sticker-handle" data-role="button" data-icon="rotate" data-iconpos="notext" data-theme="e"> </a>';setTimeout(function(){var b=function(b){b.stopPropagation();if(!a.rendered){var d=
a.element.getElementsByClassName("fx-sticker-image")[0].getBoundingClientRect();a.scale_from={x:d.left+d.width/2-5+window.scrollX,y:d.top+d.height/2-5+window.scrollY,size:SnaprFX.utils.pythag(a.element.offsetHeight,a.element.offsetWidth)/2/a.scale,scale:a.scale};a.scale_from.rotation=SnaprFX.utils.cart2polar(b.pageX-a.scale_from.x,b.pageY-a.scale_from.y)-a.rotation;c.elements.wrapper.addEventListener("mousemove",a.mousemove_scale);c.elements.wrapper.addEventListener("touchmove",a.mousemove_scale)}},
d=a.element.getElementsByClassName("fx-scale-sticker")[0];d.addEventListener("mousedown",b);d.addEventListener("touchstart",b)},4);a.element.addEventListener("dragstart",function(a){a.preventDefault()});a.element.addEventListener("touchmove",function(a){a.preventDefault()});a.deferred.resolve()})};
SnaprFX.sticker.prototype.load=function(){var b=this;if(b.load_deferred)return b.load_deferred;b.load_deferred=new Deferred;b.image=new Image;b.image.src=b.parent.sticker_pack.base_path+"assets/"+b.slug+".png";b.image.onload=function(){b.load_deferred.resolve()};return b.load_deferred};
SnaprFX.sticker.prototype.render=function(b){var c=this;c.deferred=new Deferred;var a=c.element.getElementsByClassName("fx-sticker-image")[0],d=c.element.getElementsByClassName("fx-sticker-image")[0].getBoundingClientRect(),f=c.element.parentNode.getBoundingClientRect(),h=(d.left-f.left)/f.width,g=(d.top-f.top)/f.height,e=a.offsetWidth/f.width*b.width,l=a.offsetHeight/f.height*b.height,a=c.rotation,d=Math.PI;a>2*d&&(a-=2*d);0>a&&(a+=2*d);var f=Math.sin(a),k=Math.cos(a);a<d/2?h+=f*l/b.width:a>1.5*
d?g+=-f*e/b.height:a>d/2&&a<d?(h+=(f*l+-k*e)/b.width,g+=-k*l/b.height):(h+=-k*e/b.width,g+=(-k*l-f*e)/b.height);c.load().done(function(){b.context.translate(h*b.width,g*b.height);b.context.rotate(c.rotation);b.context.drawImage(c.image,0,0,c.image.width,c.image.height,0,0,e,l);b.context.rotate(-c.rotation);b.context.translate(-h*b.width,-g*b.height);c.deferred.resolve()});return c.deferred};SnaprFX.sticker.prototype.unrender=function(){this.rendered=!1;dom.removeClass(this.element,"fx-sticker-rendered")};
SnaprFX.sticker.prototype.rerender=function(){this.rendered=!0;dom.addClass(this.element,"fx-sticker-rendered")};SnaprFX.sticker.prototype.remove=function(){var b=this;b.parent.elements.overlay.removeChild(b.element);b.parent.elements.wrapper.removeEventListener("mousemove",b.mousemove);b.parent.elements.wrapper.removeEventListener("mouseup",b.mouseup);b.parent.stickers.forEach(function(c,a){c==b&&b.parent.stickers.splice(a,1)})};
SnaprFX.sticker.prototype.get_dimensions=function(){return SnaprFX.utils.rotated_dimensions(this.element.offsetWidth,this.element.offsetHeight,-this.rotation)};SnaprFX.Canvas=function(b){var c=this;debug_logging&&console.log("canvas",b);c.options=b;c.deferred=new Deferred;c.canvas=document.createElement("canvas");c.context=c.canvas.getContext("2d");$&&debug_canvas&&($(document.body).append(c.canvas),$(c.canvas).css({border:"1px solid #f00",width:200}));if(b.url){debug_logging&&console.time("get image");switch(b.orientation){case 3:c.options.rotation=Math.PI;break;case 6:c.options.rotation=0.5*Math.PI;break;case 8:c.options.rotation=1.5*Math.PI;break;default:c.options.rotation=
...
SnaprFX.filters.curves.prototype.CubicSpline=function(){function b(c,a){var b,f,h,g,e,l,k,m,n,p;if(null!==c&&null!==a){k=c.length-1;g=[];n=[];l=[];m=[];p=[];f=[];b=[];h=[];this.max_out=a[0];this.min_out=a[0];this.max_in=c[0];this.min_in=c[0];for(e=0;0<=k?e<k:e>k;0<=k?e+=1:e-=1)g[e]=c[e+1]-c[e],this.max_out=a[e+1]>this.max_out?a[e+1]:this.max_out,this.min_out=a[e+1]<this.min_out?a[e+1]:this.min_out,this.max_in=c[e+1]>this.max_in?c[e+1]:this.max_in,this.min_in=c[e+1]<this.min_in?c[e+1]:this.min_in;
for(e=1;1<=k?e<k:e>k;1<=k?e+=1:e-=1)n[e]=3/g[e]*(a[e+1]-a[e])-3/g[e-1]*(a[e]-a[e-1]);l[0]=1;m[0]=0;p[0]=0;for(e=1;1<=k?e<k:e>k;1<=k?e+=1:e-=1)l[e]=2*(c[e+1]-c[e-1])-g[e-1]*m[e-1],m[e]=g[e]/l[e],p[e]=(n[e]-g[e-1]*p[e-1])/l[e];l[k]=1;p[k]=0;f[k]=0;for(e=l=k-1;0>=l?0>=e:0<=e;0>=l?e+=1:e-=1)f[e]=p[e]-m[e]*f[e+1],b[e]=(a[e+1]-a[e])/g[e]-g[e]*(f[e+1]+2*f[e])/3,h[e]=(f[e+1]-f[e])/(3*g[e]);this.x=c.slice(0,k+1);this.a=a.slice(0,k);this.b=b;this.c=f.slice(0,k);this.d=h}}b.prototype.derivative=function(){var c,
...
SnaprFX.filters.text.prototype.update=function(b,c){var a=this;(!1!==c.options.render_text||c.render_options.render_text||c.render_options.output)&&(!c.render_options.editable||c.options.disable_text_edit)?a.rerender():a.unrender();a.spec=b;a.draggable=b.position&&b.position.draggable;a.deferred=new Deferred;a.canvas.set_size({width:c.canvas.width,height:c.canvas.height}).done(function(){a.canvas.clear();a.calculate_position(b,c);a.create_overlay(b,c);a.text=a.text_element.innerText;a.text_element.innerHTML=
a.text.replace(/\n/g,"<br>");a.text_style.fillStyle=a.text_element.style.color;a.text_style.textAlign=a.element.style.textAlign;a.render_scale=a.canvas.height/c.elements.overlay.offsetHeight;a.set_canvas_font();a.canvas.context.textAlign=a.text_style.textAlign||"left";a.canvas.context.textBaseline="middle";a.canvas.context.fillStyle=a.text_style.fillStyle;a.text_style.fontSize=parseInt(a.text_element.style.fontSize,10);a.text_style.lineHeight=parseInt(a.text_element.style.lineHeight,10);var d=a.element.getBoundingClientRect(),
...
SnaprFX.filters.text.prototype.change_style=function(b){dom.setStyle(this.text_element,b);"text-align"in b&&(this.element.style.textAlign=b["text-align"],this.text_style.textAlign=b["text-align"]);this.check_size()};
