var JpegMeta=function(){if(this.JpegMeta)throw Error("Library included multiple times");var c=this.JpegMeta={};c.parseNum=function(b,a,c,f){var g,h;b=">"===b;void 0===c&&(c=0);void 0===f&&(f=a.length-c);for(b?g=c:g=c+f-1;b?g<c+f:g>=c;b?g++:g--)h<<=8,h+=a.charCodeAt(g);return h};c.parseSnum=function(b,a,c,f){var g,h,e;b=">"===b;void 0===c&&(c=0);void 0===f&&(f=a.length-c);for(b?g=c:g=c+f-1;b?g<c+f:g>=c;b?g++:g--)void 0===e&&(e=128===(a.charCodeAt(g)&128)),h<<=8,h+=e?~a.charCodeAt(g)&255:a.charCodeAt(g);
e&&(h=-1*(h+1));return h};c.Rational=function(b,a){this.num=b;this.den=a||1;return this};c.Rational.prototype.toString=function(){return 0===this.num||1===this.den?""+this.num:1===this.num?this.num+" / "+this.den:this.num/this.den};c.Rational.prototype.asFloat=function(){return this.num/this.den};c.MetaGroup=function(b,a){this.fieldName=b;this.description=a;this.metaProps={};return this};c.MetaGroup.prototype._addProperty=function(b,a,d){b=new c.MetaProp(b,a,d);this[b.fieldName]=b;this.metaProps[b.fieldName]=
b};c.MetaGroup.prototype.toString=function(){return"[MetaGroup "+this.description+"]"};c.MetaProp=function(b,a,c){this.fieldName=b;this.description=a;this.value=c;return this};c.MetaProp.prototype.toString=function(){return""+this.value};this.JpegMeta.JpegFile=function(b,a){var d=this._SOS;this.metaGroups={};this._binary_data=b;this.filename=a;var f=0,g=0,h,e;if(this._binary_data.slice(0,2)!==this._SOI_MARKER)throw Error("Doesn't look like a JPEG file. First two bytes are "+this._binary_data.charCodeAt(0)+
","+this._binary_data.charCodeAt(1)+".");for(f+=2;f<this._binary_data.length;){h=this._binary_data.charCodeAt(f++);e=this._binary_data.charCodeAt(f++);g=f;if(h!=this._DELIM)break;if(e===d)break;h=c.parseNum(">",this._binary_data,f,2);for(f+=h;f<this._binary_data.length;)if(h=this._binary_data.charCodeAt(f++),h==this._DELIM&&(h=this._binary_data.charCodeAt(f++),0!=h)){f-=2;break}if(h=this._markers[e]?this._markers[e][1]:void 0)this[h](e,g+2)}if(void 0===this.general)throw Error("Invalid JPEG file.");
return this};this.JpegMeta.JpegFile.prototype.toString=function(){return"[JpegFile "+this.filename+" "+this.general.type+" "+this.general.pixelWidth+"x"+this.general.pixelHeight+" Depth: "+this.general.depth+"]"};this.JpegMeta.JpegFile.prototype._SOI_MARKER="\u00ff\u00d8";this.JpegMeta.JpegFile.prototype._DELIM=255;this.JpegMeta.JpegFile.prototype._EOI=217;this.JpegMeta.JpegFile.prototype._SOS=218;this.JpegMeta.JpegFile.prototype._sofHandler=function(b,a){if(void 0!==this.general)throw Error("Unexpected multiple-frame image");
...
"W"===this.gps.GPSLongitudeRef.value&&(d=-d),this.gps._addProperty("longitude","Dec. Longitude",d)))};return this.JpegMeta}();"function"===typeof define&&define.amd&&define(function(){return JpegMeta});var debug_logging=!1,debug_canvas=!1,R=0,G=1,B=2,O=3,SnaprFX=function(c){return this.init(c)};window.SnaprFX=SnaprFX;SnaprFX.prototype.deferred=null;
SnaprFX.prototype.init=function(c){var b=this;b.deferred=$.Deferred();b.options=c;SnaprFX.utils.read_exif(b.options.url).done(function(a){b.exif=a;b.load_original(!0).done(function(){b.canvas=new SnaprFX.Canvas({width:b.original.width,height:b.original.height});b.canvas.context.drawImage(b.original.canvas,0,0);b.update_element();setTimeout(function(){b.create_overlay_elements();b.deferred.resolve()},1)})});b.load_filter_pack=$.Deferred();$.ajax({url:b.options.filter_pack+"filter-pack.json",success:function(a){b.filter_pack=
a.filter_pack;b.filter_pack.base_path=b.options.filter_pack;b.load_filter_pack.resolve(b.filter_pack);b.load_fonts()}});b.load_sticker_pack=$.Deferred();$.ajax({url:b.options.sticker_pack+"sticker-pack.json",success:function(a){b.sticker_pack=a.sticker_pack;b.sticker_pack.by_slug={};b.sticker_pack.base_path=b.options.sticker_pack;$.each(b.sticker_pack.sections,function(a,c){$.each(c.stickers,function(a,c){b.sticker_pack.by_slug[c.slug]=c})});b.load_sticker_pack.resolve(b.sticker_pack)}});b.filter_specs=
{};b.stickers=[];b.text=[]};SnaprFX.prototype.set_url=function(c,b){var a=this;this.options.url=c;var d=$.Deferred();SnaprFX.utils.read_exif(a.options.url).done(function(b){a.exif=b;a.load_original(!0).done(function(){a.apply_filter({editable:a.render_options&&a.render_options.editable});d.resolve()})});return d};
...
SnaprFX.prototype.apply_filter=function(c){function b(){var b=a.filter_specs[a.current_filter];b&&(b.layer_index=-1);if(a.render_options.region)debug_logging&&console.log("region",a.render_options.region),b=a.render_options.region,a.canvas.context.drawImage(a.original.canvas,b.left,b.top,b.width,b.height,b.left,b.top,b.width,b.height);else{var f=!1,b=function(){f&&a.canvas.context.drawImage(a.original.canvas,0,0);f=!0};a.canvas.set_size(a.render_options.width,a.render_options.height).done(b);a.original.set_size(a.render_options.width,
a.render_options.height).done(b)}b=function(){a.pixels=a.canvas.get_data();a.apply_next_layer()};!c.editable||a.options.disable_sticker_edit?a.render_stickers().done(b):b()}var a=this;$(document.body).addClass("fx-processing");c=$.extend({filter:a.current_filter,editable:!1,width:a.options.width,height:a.options.height},c);a.render_options=c;a.deferred=$.Deferred();c.editable||a.deferred.done(function(){(!1!==a.options.render_text||a.render_options.render_text)&&$.each(a.text,function(a,b){b.rerender()});
$.each(a.stickers,function(a,b){b.rerender()})});c.filter!=a.current_filter&&a.elements.overlay.find(".fx-text").remove();debug_logging&&console.group(c.filter);(a.current_filter=c.filter)?c.filter in a.filter_specs?setTimeout(b,4):$.ajax({url:a.filter_pack.base_path+"filters/"+c.filter+"/filter.json",success:function(d){debug_logging&&console.log("loaded",c.filter);a.filter_specs[c.filter]=d.filter;setTimeout(b,4)}}):setTimeout(b,4)};
SnaprFX.prototype.output=function(c){var b=this,a=$.Deferred();b.apply_filter({output:!0,width:c.width,height:c.height});b.deferred.done(function(){a.resolve(b.canvas.get_data_url())});return a};
SnaprFX.prototype.apply_next_layer=function(){var c=this;if(c.current_filter){var b=c.filter_specs[c.current_filter];b.layer_index++;if(b.layer_index>=b.layers.length)c.finish();else{debug_logging&&console.time("applying "+b.layers[b.layer_index].type+" layer");debug_logging&&console.log("applying",b.layers[b.layer_index].type,"layer");var a=b.layers[b.layer_index];a.filter?a.filter.update&&a.filter.update(a,c):a.filter=new SnaprFX.filters[a.type](a,c);a.blender=a.blender||new SnaprFX.Blender(a.blending_mode||
"normal");a.filter.deferred.done(function(){function d(){for(var d=c.render_options.region||{left:0,top:0,width:c.canvas.width,height:c.canvas.height},h,k=d.top;k<d.top+d.height;k+=1)for(var n=d.left;n<d.left+d.width;n+=1){h=4*(k*c.canvas.width+n);var m;m=a.filter.whole_canvas?[f[h],f[h+1],f[h+2],f[h+3]]:a.filter.process(h,[c.pixels[h],c.pixels[h+1],c.pixels[h+2]]);var p=m[O],p=0<=p?p/255:1;a.mask_image&&(p*=g[h]/255);p*=a.opacity/100;m=a.blender.process([c.pixels[h],c.pixels[h+1],c.pixels[h+2]],
[m[R],m[G],m[B]],p);c.pixels[h]=m[R];c.pixels[h+1]=m[G];c.pixels[h+2]=m[B]}debug_logging&&console.timeEnd("applying "+b.layers[b.layer_index].type+" layer");c.apply_next_layer()}var f;a.filter.whole_canvas&&(0!==b.layer_index&&c.canvas.put_data(c.pixels),a.filter.process(c.canvas),f=c.canvas.get_data());var g;if(a.mask_image){var h=new SnaprFX.Canvas({url:c.filter_pack.base_path+"filters/"+b.slug+"/"+a.mask_image,width:c.canvas.width,height:c.canvas.height});h.deferred.done(function(){g=h.get_data();
d()})}else d()})}}else c.finish()};SnaprFX.prototype.finish=function(){debug_logging&&console.time("writing data back");this.canvas.put_data(this.pixels);this.render_options.output||this.update_element();this.deferred.resolve();$(document.body).removeClass("fx-processing");debug_logging&&console.timeEnd("writing data back");debug_logging&&console.groupEnd(this.current_filter)};
SnaprFX.prototype.unrender_editables=function(){this.options.disable_sticker_edit||$.each(this.stickers,function(c,b){b.unrender()});this.options.disable_text_edit||$.each(this.text,function(c,b){b.unrender()});this.apply_filter({editable:!0})};SnaprFX.prototype.rerender_editables=function(){this.apply_filter({editable:!1})};
...
height:c.elements.image.height(),width:c.elements.image.width()});c.elements.wrapper.insertAfter(c.elements.image);c.elements.image.appendTo(c.elements.wrapper);c.elements.overlay.appendTo(c.elements.wrapper);c.elements.overlay.offset_cache=c.elements.overlay.offset()};"function"===typeof define&&define.amd&&define(function(){return SnaprFX});SnaprFX.utils={rotated_dimensions:function(c,b,a){var d=Math.abs(Math.cos(a)*c)+Math.abs(Math.sin(a)*b);c=Math.abs(Math.sin(a)*c)+Math.abs(Math.cos(a)*b);return{width:d,height:c}},cart2polar:function(c,b){var a=Math.atan(b/c);0<b?0>c&&(a=Math.PI+a):a=0>c?Math.PI+a:2*Math.PI+a;return a},pythag:function(c,b){return Math.sqrt(Math.pow(c,2)+Math.pow(b,2))},preload_assets:function(c){$.ajax({url:c.filter_pack+"filter-pack.json",success:function(b){$.each(b.filter_pack.sections,function(a,b){$.each(b.filters,
function(a,b){var d=c.filter_pack+"filters/"+b.slug+"/";$.ajax({url:d+"filter.json",success:function(a){a.filter.fonts&&$.each(a.filter.fonts,function(a,b){var c;c="@font-face {"+("font-family: '"+b["font-family"]+"';");b["font-weight"]&&(c+="font-weight: "+b["font-weight"]+";");b["font-style"]&&(c+="font-style: "+b["font-style"]+";");b.eot&&(c+="src: url('"+d+"fonts/"+b.eot+"');");c+="src:";b.eot&&(c+="url('"+d+"fonts/"+b.eot+"?#iefix') format('embedded-opentype'),");b.woff&&(c+="url('"+d+"fonts/"+
b.woff+"') format('woff'),");b.ttf&&(c+="url('"+d+"fonts/"+b.ttf+"') format('truetype'),");b.svg&&(c+="url('"+d+"fonts/"+b.svg+"#"+b["font-family"]+"') format('svg');");$("<style>"+(c+"}")+"</style>").appendTo(document.head);$('<span style="font-family: '+b["font-family"]+'"></span>').appendTo(document.body)});$.each(a.filter.layers,function(a,b){"image"==b.type&&((new Image).src=d+b.image.image)})}})})})}});$.ajax({url:c.sticker_pack+"sticker-pack.json",success:function(b){$.each(b.sticker_pack.sections,
function(a,b){$.each(b.stickers,function(a,b){(new Image).src=c.sticker_pack+"assets/"+b.slug+".png"})})}})},read_exif:function(c){var b=$.Deferred(),a=new XMLHttpRequest;a.open("GET",c,!0);a.responseType="arraybuffer";a.onload=function(c){var f=new FileReader;f.onloadend=function(){var c=new JpegMeta.JpegFile(this.result,"test.jpg");a=f=null;b.resolve({latitude:c.gps&&c.gps.latitude&&c.gps.latitude.value,longitude:c.gps&&c.gps.longitude&&c.gps.longitude.value,date:c.exif&&c.exif.DateTimeOriginal&&
c.exif.DateTimeOriginal.value,orientation:c.tiff&&c.tiff.Orientation&&c.tiff.Orientation.value})};c=new Blob([this.response]);f.readAsBinaryString(c)};a.send();return b},rgbToHsl:function(c,b,a){c/=255;b/=255;a/=255;var d=Math.max(c,b,a),f=Math.min(c,b,a),g,h=(d+f)/2;if(d==f)g=f=0;else{var e=d-f,f=0.5<h?e/(2-d-f):e/(d+f);switch(d){case c:g=(b-a)/e+(b<a?6:0);break;case b:g=(a-c)/e+2;break;case a:g=(c-b)/e+4}g/=6}return[g,f,h]},hslToRgb:function(c,b,a){if(0===b)a=b=c=a;else{var d=0.5>a?a*(1+b):a+b-
a*b,f=2*a-d;a=SnaprFX.utils.hueToRgb(f,d,c+1/3);b=SnaprFX.utils.hueToRgb(f,d,c);c=SnaprFX.utils.hueToRgb(f,d,c-1/3)}return[255*a,255*b,255*c]},hueToRgb:function(c,b,a){0>a&&(a+=1);1<a&&(a-=1);return a<1/6?c+6*(b-c)*a:0.5>a?b:a<2/3?c+6*(b-c)*(2/3-a):c}};SnaprFX.prototype.add_sticker=function(c){var b=this,a=new SnaprFX.sticker(c,b);b.stickers.push(a);a.deferred.done(function(){b.elements.overlay.append(a.element);a.element.addClass("fx-active").siblings(".fx-active").removeClass("fx-active")})};
...
d+='<a class="fx-scale-sticker fx-sticker-handle" data-role="button" data-icon="rotate" data-iconpos="notext" data-theme="e">R</a>';d+='<img class="fx-sticker-image" src="'+a.parent.sticker_pack.base_path+"assets/"+c+'.png">';d+="</div>";var f={position:"absolute",width:a.image.width*a.scale_factor,height:a.image.height*a.scale_factor};a.spec.position&&a.spec.position.center?(f.left=a.spec.position.center.x*a.scale_factor-f.width/2,f.top=a.spec.position.center.y*a.scale_factor-f.height/2):(f.left=
b.canvas.width/2-f.width/2,f.top=b.canvas.height/2-f.height/2);a.element=$(d).css(f);a.initial=f;a.rendered||a.element.removeClass("fx-sticker-rendered");a.element.find(".fx-render-sticker").on("click",function(a){b.rerender_editables();a.stopPropagation()});a.element.find(".fx-remove-sticker").on("click",function(){a.remove()});a.element.on("click",function(){a.parent.options.disable_sticker_edit||(a.rendered&&b.unrender_editables(),a.element.addClass("fx-active").removeClass("fx-sticker-rendered").siblings(".fx-active").removeClass("fx-active"))});
a.mousemove_drag=function(b){a.rendered||a.element.css({left:b.pageX-a.drag_from.left,top:b.pageY-a.drag_from.top})};a.mouseup=function(){b.elements.wrapper.off("mousemove",a.mousemove_drag);b.elements.wrapper.off("mousemove",a.mousemove_scale)};b.elements.wrapper.on("mouseup",a.mouseup);a.element.on("mousedown",function(c){if(!a.rendered){var d=parseInt(a.element.css("left"),10),e=parseInt(a.element.css("top"),10);a.drag_from={left:c.pageX-d,top:c.pageY-e,event:c};b.elements.wrapper.on("mousemove",
...
(a.rotation/(2*Math.PI))+"deg)",width:b,height:c,top:a.scale_from.y-c/2-a.parent.elements.overlay.offset_cache.top,left:a.scale_from.x-b/2-a.parent.elements.overlay.offset_cache.left})}};a.element.find(".fx-scale-sticker").on("mousedown",function(c){c.stopPropagation();window.element=a.element;if(!a.rendered){var d=a.get_dimensions(),e=a.element.find(".fx-sticker-image").offset();a.scale_from={x:e.left+d.width/2,y:e.top+d.height/2,size:SnaprFX.utils.pythag(a.element.height(),a.element.width())/2/
...
SnaprFX.Canvas.prototype.get_data=function(){var c=this.context.getImageData(0,0,this.canvas.width,this.canvas.height);this.data=c;return c.data};SnaprFX.Canvas.prototype.put_data=function(c){this.data.data=c;this.context.putImageData(this.data,0,0);return this};SnaprFX.Canvas.prototype.get_data_url=function(){return this.canvas.toDataURL("image/jpeg",1)};SnaprFX.Canvas.prototype.clear=function(){this.canvas.width=this.canvas.width};
SnaprFX.Canvas.prototype.set_size=function(c,b){this.canvas.width!=c&&(this.width=c,this.options.width=c,this.canvas.width=c);this.canvas.height!=b&&(this.height=b,this.options.height=b,this.canvas.height=b);return this.image?this.place_image():$.Deferred().resolve()};SnaprFX.Canvas.prototype.clone=function(c){return new SnaprFX.Canvas($.extend({},this.options,c))};SnaprFX.Blender=function(c){debug_logging&&console.log(" - Blend:",c);this.mode=this.blend_modes[c];this.process=this.mode.rgb?function(b,a,c){a=this.mode.process(b,a);for(var f=[],g=R;g<=B;g++)f[g]=a[g]*c+b[g]*(1-c);return f}:function(b,a,c){for(var f=[],g=R;g<=B;g++)f[g]=this.mode.process(b[g],a[g]),f[g]=f[g]*c+b[g]*(1-c);return f}};
...
SnaprFX.filters.image.prototype.update=function(c,b){var a=this;console.log("updating",b.canvas.width);a.width=b.canvas.width;a.height=b.canvas.height;a.deferred=$.Deferred();a.canvas.set_size(a.width,a.height).done(function(){a.pixels=a.canvas.get_data();a.deferred.resolve()})};SnaprFX.filters.image.prototype.process=function(c,b){return[this.pixels[c],this.pixels[c+1],this.pixels[c+2],this.pixels[c+3]]};var debug_borders=!1;SnaprFX.prototype.add_text=function(c){c.filter=new SnaprFX.filters.text(c,this);this.filter_specs[this.current_filter].layers.push(c)};SnaprFX.filters.text=function(c,b){b.text.push(this);this.slug=c.slug;this.text_style=c.text.style;this.text=b.options.text&&b.options.text[c.slug]||c.text.default_value;this.canvas=new SnaprFX.Canvas({width:b.canvas.width,height:b.canvas.height});this.update(c,b)};
SnaprFX.filters.text.prototype.calculate_position=function(c,b){this.x_scale_factor=this.canvas.width/b.filter_specs[b.current_filter].target_canvas.width;this.y_scale_factor=this.canvas.height/b.filter_specs[b.current_filter].target_canvas.height;this.position=c.position&&c.position.bbox?{top:c.position.bbox.top*this.y_scale_factor,bottom:c.position.bbox.bottom*this.y_scale_factor,left:c.position.bbox.left*this.x_scale_factor,right:c.position.bbox.right*this.x_scale_factor}:{top:0,bottom:this.canvas.height,
left:0,right:this.canvas.width};c.position&&(this.position.x=c.position.x*this.x_scale_factor,this.position.y=c.position.y*this.y_scale_factor);this.bbox={top:this.position.top,bottom:this.position.bottom,left:this.position.left,right:this.position.right}};
...
SnaprFX.filters.text.prototype.update=function(c,b){function a(a,b){for(var c=a.split("\n"),f=[],e=0,g=0;g<c.length;g++){var h=c[g].split(" ");f[e]=h[0];for(var l=1;l<h.length;l++)d.canvas.context.measureText(f[e]+" "+h[l]).width>b?(e++,f[e]=h[l]):f[e]=f[e]+" "+h[l];e++}return f}var d=this;d.rendered=(!1!==b.options.render_text||b.render_options.render_text||b.render_options.output)&&(!b.render_options.editable||b.options.disable_text_edit);d.spec=c;d.dragable=c.position&&c.position.dragable;d.deferred=
