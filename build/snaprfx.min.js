...
b.elements.locked=dom.div("fx-locked-layer");dom.setStyle(d,{display:"none",position:"absolute",top:0,left:0,height:"100%",width:"100%"});b.elements.image.parentNode.insertBefore(a,b.elements.image.nextSibling);a.appendChild(b.elements.image);a.appendChild(c);a.appendChild(d);debug_canvas&&dom.addClass(c,"fx-debug")};SnaprFX.prototype.on=function(b,c){this.handlers[b]||(this.handlers[b]=[]);this.handlers[b].push(c)};
...
a.parent.sticker_pack.base_path+"assets/"+b+'.png">';var d={position:"absolute",width:a.image.width*a.scale_factor,height:a.image.height*a.scale_factor};a.spec.position&&a.spec.position.x?(d.left=a.spec.position.x*a.scale_factor-d.width/2,d.top=a.spec.position.y*a.scale_factor-d.height/2):(d.left=c.canvas.width/2-d.width/2,d.top=c.canvas.height/2-d.height/2);d.width+="px";d.height+="px";d.left+="px";d.top+="px";dom.setStyle(a.element,d);a.initial=d;a.rendered||dom.removeClass(a.element,"fx-sticker-rendered");
a.element.innerHTML+='<a class="fx-render-sticker fx-sticker-handle" data-role="button" data-icon="tick" data-iconpos="notext" data-theme="e">\u2714</a>';setTimeout(function(){a.element.getElementsByClassName("fx-render-sticker")[0].addEventListener("click",function(a){c.rerender_editables();a.stopPropagation()})},4);a.element.innerHTML+='<a class="dd fx-remove-sticker fx-sticker-handle" data-role="button" data-icon="delete" data-iconpos="notext" data-theme="e">\u2717</a>';setTimeout(function(){a.element.getElementsByClassName("fx-remove-sticker")[0].addEventListener("click",
function(){a.remove()})},4);a.element.addEventListener("mousedown",function(){if(!a.parent.options.disable_sticker_edit){a.rendered&&c.unrender_editables();for(var b=c.elements.overlay.getElementsByClassName("fx-active"),d=0;d<b.length;++d)dom.removeClass(b[d],"fx-active");dom.addClass(a.element,"fx-active");dom.removeClass(a.element,"fx-sticker-rendered")}});a.mousemove_drag=function(c){a.rendered||dom.setStyle(a.element,{left:c.clientX-a.drag_from.left+"px",top:c.clientY-a.drag_from.top+"px"})};
a.mouseup=function(){c.elements.wrapper.removeEventListener("mousemove",a.mousemove_drag);c.elements.wrapper.removeEventListener("mousemove",a.mousemove_scale)};c.elements.wrapper.addEventListener("mouseup",a.mouseup);a.element.addEventListener("mousedown",function(b){if(!a.rendered){var d=parseInt(a.element.style.left,10),g=parseInt(a.element.style.top,10);a.drag_from={left:b.clientX-d,top:b.clientY-g,event:b};c.elements.wrapper.addEventListener("mousemove",a.mousemove_drag)}dom.addClass(c.elements.overlay,
"fx-editing-sticker");dom.removeClass(c.elements.overlay,"fx-editing-text")});a.mousemove_scale=function(c){c.stopPropagation();if(!a.rendered){var b=SnaprFX.utils.pythag(c.clientX-a.scale_from.x,c.clientY-a.scale_from.y);a.scale=b/a.scale_from.size;a.scale=Math.max(a.scale,0.15);c=SnaprFX.utils.cart2polar(c.clientX-a.scale_from.x,c.clientY-a.scale_from.y);a.rotation=-a.scale_from.rotation+c;c=parseInt(a.initial.width,10)*a.scale;var b=parseInt(a.initial.height,10)*a.scale,d=a.parent.elements.overlay.getBoundingClientRect();
dom.setStyle(a.element,{webkitTransform:"rotate("+Math.round(360*(a.rotation/(2*Math.PI)))+"deg)",transform:"rotate("+Math.round(360*(a.rotation/(2*Math.PI)))+"deg)",width:c+"px",height:b+"px",top:a.scale_from.y-b/2-d.top-window.pageXOffset+"px",left:a.scale_from.x-c/2-d.left-window.pageYOffset+"px"})}};a.element.innerHTML+='<a class="fx-scale-sticker fx-sticker-handle" data-role="button" data-icon="rotate" data-iconpos="notext" data-theme="e"> </a>';setTimeout(function(){a.element.getElementsByClassName("fx-scale-sticker")[0].addEventListener("mousedown",
function(b){b.stopPropagation();if(!a.rendered){var d=a.element.getElementsByClassName("fx-sticker-image")[0].getBoundingClientRect();a.scale_from={x:d.left+d.width/2-5,y:d.top+d.height/2-5,size:SnaprFX.utils.pythag(a.element.offsetHeight,a.element.offsetWidth)/2/a.scale,scale:a.scale};a.scale_from.rotation=SnaprFX.utils.cart2polar(b.clientX-a.scale_from.x,b.clientY-a.scale_from.y)-a.rotation;c.elements.wrapper.addEventListener("mousemove",a.mousemove_scale)}})},4);a.element.addEventListener("dragstart",
function(a){a.preventDefault()});a.deferred.resolve()})};SnaprFX.sticker.prototype.load=function(){var b=this;if(b.load_deferred)return b.load_deferred;b.load_deferred=new Deferred;b.image=new Image;b.image.src=b.parent.sticker_pack.base_path+"assets/"+b.slug+".png";b.image.onload=function(){b.load_deferred.resolve()};return b.load_deferred};
...
d?g+=-f*e/b.height:a>d/2&&a<d?(h+=(f*l+-k*e)/b.width,g+=-k*l/b.height):(h+=-k*e/b.width,g+=(-k*l-f*e)/b.height);console.warn(h,g,h*b.width,g*b.height);c.load().done(function(){b.context.translate(h*b.width,g*b.height);b.context.rotate(c.rotation);b.context.drawImage(c.image,0,0,c.image.width,c.image.height,0,0,e,l);b.context.rotate(-c.rotation);b.context.translate(-h*b.width,-g*b.height);c.deferred.resolve()});return c.deferred};
