...
SnaprFX.filters.text.prototype.update=function(b,c){var a=this;(!1!==c.options.render_text||c.render_options.render_text||c.render_options.output)&&(!c.render_options.editable||c.options.disable_text_edit)?a.rerender():a.unrender();a.spec=b;a.draggable=b.position&&b.position.draggable;a.deferred=new Deferred;a.canvas.set_size({width:c.canvas.width,height:c.canvas.height}).done(function(){function d(b,c){for(var d=b.split("\n"),f=[],h=0,e=0;e<d.length;e++){var g=d[e].split(" ");f[h]=g[0];for(var k=
1;k<g.length;k++)g[k]&&(a.canvas.context.measureText(f[h]+" "+g[k]).width>c?(h++,f[h]=g[k]):f[h]=f[h]+" "+g[k]);h++}return f}a.canvas.clear();a.calculate_position(b,c);a.create_overlay(b,c);a.text=a.text_element.innerText;a.text_element.innerHTML=a.text.replace(/\n/g,"<br>");a.text_style.fillStyle=a.text_element.style.color;a.text_style.textAlign=a.element.style.textAlign;a.render_scale=a.canvas.height/c.elements.overlay.offsetHeight;a.set_canvas_font();a.canvas.context.textAlign=a.text_style.textAlign||
"left";a.canvas.context.textBaseline=a.text_style.textBaseline||"top";a.canvas.context.fillStyle=a.text_style.fillStyle;a.text_style.fontSize=parseInt(a.text_element.style.fontSize,10);a.text_style.lineHeight=parseInt(a.text_element.style.lineHeight,10);var f=a.element.getBoundingClientRect(),h=c.elements.overlay.getBoundingClientRect();a.position.left=parseInt(f.left,10)-parseInt(h.left,10);a.position.top=parseInt(f.top,10)-parseInt(h.top,10);a.position.right=a.position.left+parseInt(f.width,10);
a.position.bottom=a.position.top+parseInt(f.height,10);for(var f=a.position.right-a.position.left,g=Math.round(a.position.bottom-a.position.top),h=d(a.text,f),e=1E3;e&&(!h||(h.length-1)*a.text_style.lineHeight+a.text_style.fontSize>g);)e--,a.text_style.fontSize*=0.8,a.text_element.style.fontSize=a.text_style.fontSize+"px",a.text_style.lineHeight*=0.8,a.text_element.style.lineHeight=a.text_style.lineHeight+"px",a.set_canvas_font(),h=d(a.text,f);e||console.warn("render shrunk text 1000 times without success!");
switch(a.text_style.textAlign){case "end":case "right":e=a.position.right;break;case "center":e=f/2+a.position.left;break;default:e=a.position.left}e*=a.render_scale;switch(a.text_style.textBaseline){case "alphabetic":case "ideographic":case "bottom":g=a.position.bottom-a.text_style.lineHeight*(h.length-1);break;case "middle":g=g/2+a.position.top-a.text_style.lineHeight*(h.length-1)/2;break;default:g=a.position.top}g*=a.render_scale;if(a.slug!==c.render_options.active_text&&!b.removed)for(var k=0;k<
h.length;k++)debug_borders&&(a.canvas.context.strokeStyle="#ff0000",a.canvas.context.strokeRect(e,g+k*a.text_style.lineHeight*a.render_scale,f*a.render_scale,a.text_style.lineHeight*a.render_scale)),debug_logging&&console.log("drawing text",h[k],e,g+k*a.text_style.lineHeight*a.render_scale,f*a.render_scale),a.canvas.context.fillText(h[k],e,g+k*a.text_style.lineHeight*a.render_scale,f*a.render_scale);a.pixels=a.canvas.get_data();a.deferred.resolve()})};
