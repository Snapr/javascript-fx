...
0,0);f=!0};b.canvas.set_size(b.render_options.width,b.render_options.height).done(a);b.original.set_size(b.render_options.width,b.render_options.height).done(a)}a=function(){b.pixels=b.canvas.get_data();b.apply_next_layer()};!c.editable||b.options.disable_sticker_edit?b.render_stickers().done(a):a()}c=$.extend({filter:b.current_filter,editable:!1,width:b.options.width,height:b.options.height},c);b.render_options=c;c.editable||b.deferred.done(function(){(!1!==b.options.render_text||b.render_options.render_text)&&
$.each(b.text,function(a,b){b.rerender()});$.each(b.stickers,function(a,b){b.rerender()})});c.filter!=b.current_filter&&b.elements.overlay.find(".fx-text").addClass("fx-text-old");debug_logging&&console.group(c.filter);(b.current_filter=c.filter)?c.filter in b.filter_specs?setTimeout(a,4):$.ajax({url:b.filter_pack.base_path+"filters/"+c.filter+"/filter.json",success:function(d){debug_logging&&console.log("loaded",c.filter);b.filter_specs[c.filter]=d.filter;setTimeout(a,4)}}):setTimeout(a,4)},4)};
...
c.render_options.region||{left:0,top:0,width:c.canvas.width,height:c.canvas.height},h,k=d.top;k<d.top+d.height;k+=1)for(var n=d.left;n<d.left+d.width;n+=1){h=4*(k*c.canvas.width+n);var m;m=a.filter.whole_canvas?[f[h],f[h+1],f[h+2],f[h+3]]:a.filter.process(h,[c.pixels[h],c.pixels[h+1],c.pixels[h+2]]);var p=m[O],p=0<=p?p/255:1;a.mask_image&&(p*=g[h]/255);p*=a.opacity/100;m=a.blender.process([c.pixels[h],c.pixels[h+1],c.pixels[h+2]],[m[R],m[G],m[B]],p);c.pixels[h]=m[R];c.pixels[h+1]=m[G];c.pixels[h+
2]=m[B]}debug_logging&&console.timeEnd("applying "+b.layers[b.layer_index].type+" layer");c.apply_next_layer()}var f;a.filter.whole_canvas&&(0!==b.layer_index&&c.canvas.put_data(c.pixels),a.filter.process(c.canvas),f=c.canvas.get_data());var g;if(a.mask_image){var h=new SnaprFX.Canvas({url:c.filter_pack.base_path+"filters/"+b.slug+"/"+a.mask_image,width:c.canvas.width,height:c.canvas.height});h.deferred.done(function(){g=h.get_data();d()})}else d()})}};
SnaprFX.prototype.finish=function(){debug_logging&&console.time("writing data back");this.elements.overlay.find(".fx-text-old").remove();this.canvas.put_data(this.pixels);this.render_options.output||this.update_element();this.deferred.resolve();$(document.body).removeClass("fx-processing");debug_logging&&console.timeEnd("writing data back");debug_logging&&console.groupEnd(this.current_filter)};
